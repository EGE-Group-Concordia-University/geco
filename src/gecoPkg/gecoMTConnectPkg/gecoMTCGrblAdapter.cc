...
int gecoMTCGrblAdapter::cmd(int &i,int objc,Tcl_Obj *const objv[])
{
  ...
  if (index==getOptionIndex("-grbl"))
      ...
      if ((status==Active) && (objc==3))
	{
      ...
      if (objc==2)
	{
	  if (grblConnected) Tcl_AppendResult(interp, Tcl_GetChannelName(getTclChannel()), NULL);
	  i++;
	}
	  ...
	  if (getline(gcodeFile, block))
	    {
	      ...
	      if (sendGcode(block.c_str())!=0) gcodeFile.close();
	    }
	  else
	    {
...
{
  if (!grblChan) return gecoMTCAdapter::SHDR(forceSend);
...
int gecoMTCGrblAdapter::handleGrblResponse()
{
  ...
  if (strstr(Tcl_GetStringResult(getTclInterp()), "<")) return 0;
